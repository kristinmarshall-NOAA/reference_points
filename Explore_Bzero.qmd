...
```
The 3-panel plot that's in the paper (doesn't have darkblotched rockfish and the y-axis is on a log-scale) and a table of corresponding values:
```{r, echo=F}
replot2<- ref2 %>% 
...
Here is a summary table of percent change from max to min value of estimated spawning output  
```{r}
refsum<- replot2 %>%
  group_by(Stock.Name) %>% 
